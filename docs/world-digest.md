...
- AC2: the briefing marks arrivals (a name the persisted speaker roster
  has never seen — never inferred from bounded feed history, which would
  call a returning resident new) and silences (spoke before the window,
  not within it) so the digest can name them.
...
- AC4: short paragraphs, each opening with a bold header phrase; fits one
  Discord message.
- AC5: state survives restarts atomically; a restart never double-posts a
  window. Delivery is at-most-once: a crash between the webhook call and
  the completion record treats the window as covered rather than risk a
  duplicate; a known-refused post retries next interval. After prolonged
  failure the walk reaches back to the cursor, however far, so a late
  chronicle covers its whole window.
- AC6: a missing webhook or a non-positive interval refuses to start,
